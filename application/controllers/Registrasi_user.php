...
class Registrasi_user extends CI_controller
{
	...
	function tambah_registrasi(){
		...
		$data['sub_title'] = "Tambah Registrasi";

		$this->load->view('template2/header', $data);
		$this->load->view('admin/tambah_registrasi_user', $data);
		$this->load->view('template2/footer');
		...
		if ($this->input->post('kirim')) {
			...
			$this->db->insert('tbl_registrasi_user', $data);

			$this->session->set_flashdata('message', 'swal("Sukses!", "Data berhasil ditambah", "success");');
			 redirect('Registrasi_user/tambah_registrasi');
		}
	}
